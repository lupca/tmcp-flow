...
- **Pages chính:**
  ...
  - `Studio.jsx`: Giao diện thiết kế (Canvas) đã được module hóa (tách hàm, tách logic ra các file con). Bao gồm:
    ...
    - **Camera Focus Selection:** Toggle trong FX để bật/tắt hiệu ứng highlight node theo camera khi preview/render.
...
2. Frontend gọi POST đến Backend Render `/api/render-stream` (Port 3000), đưa theo State chứa: `nodes`, `edges`, `cameraSequence`, cùng config (resolution, framerate) và các hiệu ứng `edgeEffectType`, `selectionEffect`.
3. Nếu bật **Camera Focus Selection**, frontend gửi thêm `renderSelectionEffect` để kích hoạt highlight node theo camera trong video.
4. Endpoint Express mở `selectComposition()` của Remotion để đo độ phân giải/thời lượng.
5. Gọi hàm cốt lõi `@remotion/renderer/renderMedia` có tối ưu hóa phần cứng (HW VideoToolbox, chia nhỏ CPU Threads, GPU Angle) dựa vào máy chủ đang đặt OS.
6. Trình duyệt Headless Chromium mở Component React tĩnh `DynamicFlowScene.jsx` ảo với state JSON nhận được, quay lại hành vi ReactFlow, chụp hình 60 frame JPEG mỗi giây. Liên tục ping ngược tiến trình (%) về cho Studio.jsx qua kết nối Event Source.
7. Khi `renderSelectionEffect` bật, `DynamicFlowScene` chọn node sáng theo **camera timeline** (ưu tiên mốc `startFrame` của edge nếu có, fallback theo `cameraSequence`).
8. Gom các bức ảnh và đóng gói encode MP4 h264 tại folder `out/`.
9. Gửi URL file tải về cục bộ.
...
- `src/`
  ...
  - `components/DynamicFlowScene.jsx`: Trình đọc state trung gian Wrapper "Trơ" (Vô hiệu hóa UI Kéo thả, chỉ hiển thị UI tĩnh) chuyên dùng để pass vào `Remotion Root` chụp hình.
    - Hỗ trợ highlight node theo camera (`renderSelectionEffect`) và sync theo timeline.
